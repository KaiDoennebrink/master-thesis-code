...
def apply_standardizer(data, standard_scaler: StandardScaler) -> np.ndarray:
    ...
    for x in data:
        x_shape = x.shape
        x_tmp.append(
            standard_scaler.transform(x.flatten()[:, np.newaxis]).reshape(x_shape)
        )
    return np.array(x_tmp).astype(np.float32)


def resample_multichannel_signal(
    orig_signal: np.ndarray, orig_frequency: int = 500, target_frequency: int = 100
) -> np.ndarray:
    ...
    for channel in range(orig_signal.shape[1]):
        resampled_x, _ = resample_sig(
            orig_signal[:, channel], orig_frequency, target_frequency
        )
