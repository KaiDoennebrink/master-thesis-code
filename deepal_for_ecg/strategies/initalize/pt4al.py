...
class PreTextLossInitQueryStrategy:
    ...
    def __init__(
        ...
        name: str,
        loss_storage_dir: Path = Path("./data/loss_storage"),
        transformation_dir: Path = Path(
            "./data/saved/transformation_recognition/transform"
        ),
    ):
        self._pretrained_model = pretrained_model
        self._loss_storage_dir = Path(loss_storage_dir, name)
    ...
    def prepare(
        ...
        if load_losses:
            ...
            self._loss_indices = np.load(
                Path(self._loss_storage_dir, "loss_indices.npy")
            )
        else:
            ...
            if save_losses:
                ...
                save_numpy(
                    self._loss_indices, Path(self._loss_storage_dir, "loss_indices.npy")
                )
    ...
    def select_samples(
        self,
        num_of_samples: int,
        previous_round_model: keras.Model = None,
        current_batch: int = -1,
    ) -> Set[int]:
        """
    ...
    def _select_uniformly_from_batch(
        self, num_of_samples: int, current_batch: int
    ) -> Set[int]:
    ...
    def _calculate_losses(self, training_samples: np.ndarray) -> np.ndarray:
        ...
        expected_model_output = np.array(
            [
                Transformation.ORIG.one_hot_label,
                Transformation.ADD_NOISE.one_hot_label,
                Transformation.SCALE.one_hot_label,
                Transformation.NEGATE.one_hot_label,
        ...
        for i, model_input in tqdm(
            enumerate(model_inputs_base), desc="Calculating losses", total=num_samples
        ):
    ...
    def _get_transformation_data(
        self, training_samples: np.ndarray
    ) -> (Tuple)[
        ...
        if self._transformation_dir.exists():
            ...
            noisy_data = np.load(
                Path(
                    self._transformation_dir,
                    f"{Transformation.ADD_NOISE.data_name}.npy",
                )
            )
            scaled_data = np.load(
                Path(self._transformation_dir, f"{Transformation.SCALE.data_name}.npy")
            )
            negation_data = np.load(
                Path(self._transformation_dir, f"{Transformation.NEGATE.data_name}.npy")
            )
            temporal_inversed_data = np.load(
                Path(
                    self._transformation_dir,
                    f"{Transformation.TEMPORAL_INVERSE.data_name}.npy",
                )
            )
            permuted_data = np.load(
                Path(
                    self._transformation_dir,
                    f"{Transformation.PERMUTATION.data_name}.npy",
                )
            )
            time_warped_data = np.load(
                Path(
                    self._transformation_dir,
                    f"{Transformation.TIME_WARPING.data_name}.npy",
                )
            )
            return (
                noisy_data,
